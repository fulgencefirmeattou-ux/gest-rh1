...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ── 4. Départements (sans responsable pour l'instant) ──────
        $depsData = [
        ...
        $dg = User::firstOrCreate(['email' => 'dg@gestRH.com'], [
            'nom'      => 'Konan',
        ...
        $respServicesData = [
            'Informatique'        => ['email' => 'resp.dsi@gestRH.com',      'nom' => 'Kra',   'prenom' => 'Franck',   'login' => 'RSP000001'],
            'Finance'             => ['email' => 'resp.finance@gestRH.com',  'nom' => 'Touré', 'prenom' => 'Bakary',   'login' => 'RSP000002'],
            'Ressources Humaines' => ['email' => 'resp.rh@gestRH.com',       'nom' => 'Adou',  'prenom' => 'Cécile',   'login' => 'RSP000003'],
            'Marketing'           => ['email' => 'resp.mkt@gestRH.com',      'nom' => 'N\'Da', 'prenom' => 'Parfait',  'login' => 'RSP000004'],
            'Opérations'          => ['email' => 'resp.ops@gestRH.com',      'nom' => 'Séka',  'prenom' => 'Romuald',  'login' => 'RSP000005'],
        ];

        $respUsers = [];
        foreach ($respServicesData as $depNom => $data) {
            ...
            $user->syncRoles(['responsable-service']);
            $respUsers[$depNom] = $user;
        }

        // ── 7. Services ────────────────────────────────────────────
        $servicesData = [
            'Informatique'        => 'Service DSI',
            'Finance'             => 'Service Comptabilité & Finance',
            'Ressources Humaines' => 'Service RH & Administration',
            'Marketing'           => 'Service Marketing & Commercial',
            'Opérations'          => 'Service Logistique & Opérations',
        ];

        $services = [];
        foreach ($servicesData as $depNom => $serviceNom) {
            $dep     = Departement::where('nom', $depNom)->first();
            $resp    = $respUsers[$depNom] ?? null;
            $service = Service::firstOrCreate(['nom' => $serviceNom], [
                'description'    => "Service rattaché au département {$depNom}",
                'departement_id' => $dep->id,
                'responsable_id' => $resp?->id,
                'date_creation'  => now(),
            ]);
            $services[$depNom] = $service;
        }

        // Affecter les responsables aux départements
        foreach ($respUsers as $depNom => $user) {
            Departement::where('nom', $depNom)->update(['responsable_id' => $user->id]);
        }
        ...
        foreach ($employes as $index => $data) {
            ...
            $service = $services[$data['dep']] ?? null;
        ...
        // ── 12. Demandes de congé ─────────────────────────────────
        $this->call(DemandeCongeSeeder::class);
        ...
        $this->command->info('  dg@gestRH.com           → password  (dg)');
        $this->command->info('  resp.dsi@gestRH.com     → password  (responsable_service)');
        $this->command->info('  resp.finance@gestRH.com → password  (responsable_service)');
        $this->command->info('  resp.rh@gestRH.com      → password  (responsable_service)');
        $this->command->info('  resp.mkt@gestRH.com     → password  (responsable_service)');
        $this->command->info('  resp.ops@gestRH.com     → password  (responsable_service)');
