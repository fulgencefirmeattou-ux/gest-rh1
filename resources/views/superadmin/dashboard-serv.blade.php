@extends('layouts.app')
@section('title', 'Tableau de bord — Responsable service')
...
<div class="container-fluid">
            ...
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <div>
                    <h4 class="page-title mb-1">
                        Tableau de bord
                        @if($service)
                            <span class="badge ms-2 fs-12" style="background:#47ad77;">{{ $service->nom }}</span>
                        @endif
                    </h4>
    ...
    @if(!$service)
        <div class="alert alert-warning">
            <i class="ri-error-warning-line me-2"></i>Aucun service ne vous est assigné comme responsable.
        </div>
    @else
        ...
        <div class="col-sm-4">
            <div class="card widget-flat h-100" style="background:linear-gradient(135deg,#47ad77,#2e8b57);">
                <div class="card-body text-white">
                    ...
                    <h6 class="text-uppercase mt-0 fw-semibold opacity-75">Employés du service</h6>
                    <h2 class="my-2 fw-bold">{{ $totalEmployesServ }}</h2>
                    <p class="mb-0 small">{{ $service->nom }}</p>
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="card widget-flat h-100" style="background:linear-gradient(135deg,#f77e53,#e05b2d);">
                <div class="card-body text-white">
                    ...
                    <p class="mb-0 small">En attente dans votre service</p>
                </div>
                <a href="{{ route('conges.approbation.service') }}" class="card-footer bg-white bg-opacity-10 text-white text-decoration-none d-flex align-items-center justify-content-between px-3 py-2 small">
        ...
        <div class="col-lg-5">
            <div class="card h-100">
                <div class="card-header bg-transparent d-flex align-items-center justify-content-between pt-3">
                    ...
                    <a href="{{ route('conges.approbation.service') }}" class="btn btn-sm btn-outline-success">Traiter</a>
                </div>
        ...
        <div class="col-lg-7">
            <div class="card h-100">
                <div class="card-header bg-transparent pt-3">
                    <h5 class="card-title mb-0"><i class="ri-group-line me-2 text-success"></i>Employés du service</h5>
                </div>
                <div class="card-body p-0">
                    @if($employesServ->isEmpty())
                        <div class="text-center text-muted py-5">
                    ...
                    @else
                        <div class="table-responsive">
                            <table class="table table-sm table-hover mb-0 align-middle">
                                <thead class="table-light">
                                    <tr><th class="ps-3">Nom</th><th>Poste</th></tr>
                                </thead>
                                <tbody>
                                    @foreach($employesServ as $emp)
                                    <tr>
                                        <td class="ps-3">
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="rounded-circle bg-success-subtle d-flex align-items-center justify-content-center" style="width:32px;height:32px;min-width:32px;">
                                                    <span class="text-success fw-bold small">{{ strtoupper(substr($emp->prenom,0,1)) }}{{ strtoupper(substr($emp->nom,0,1)) }}</span>
                                                </div>
