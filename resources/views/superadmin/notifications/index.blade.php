    ...
    <div class="card shadow-sm">
        <div class="card-body p-0">
            @forelse($notifications as $notif)
                @php
                    $data = $notif->data;
                    $hasUrl = !empty($data['url']) && $data['url'] !== '#';
                    $goUrl  = route('notifications.go', $notif->id);
                @endphp
                <a href="{{ $goUrl }}"
                   class="d-flex align-items-start p-3 border-bottom text-decoration-none text-dark {{ $notif->read_at ? '' : 'bg-light' }}"
                   style="{{ $hasUrl ? '' : 'pointer-events:none;' }}">
                    <div class="me-3">
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex justify-content-between">
                            ...
                            <small class="text-muted ms-2 text-nowrap">{{ $notif->created_at->diffForHumans() }}</small>
                        </div>
                        <p class="mb-0 small text-muted">{{ $data['message'] ?? '' }}</p>
                        @if($hasUrl)
                            <small class="text-primary"><i class="ri-arrow-right-line"></i> Voir</small>
                        @endif
                    </div>
                    @if(!$notif->read_at)
                        <span class="badge bg-primary ms-2 align-self-center">Nouveau</span>
                    @endif
                </a>
            @empty
